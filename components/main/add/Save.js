...
import { getFirestore, collection, query, limit, doc, where, addDoc, getDocs, FieldValue } from 'firebase/firestore';
...
function Save(props) {
    ...
    const savePostData = async (downloadURL, downloadURLStill) => {
        ...
        if (array !== null) {
            ...
            for (let i = 0; i < array.length; i++) {
                ...
                const q= query(usersCollectionRef, where("username", "==", array[i].substring(1)));
    ...
    const callback = async (keyword) => {
        setKeyword(keyword);
        const usersCollectionRef= collection(db,"users");
        const q= query(usersCollectionRef, where("username", ">=", keyword.substring(1)), limit(10));
