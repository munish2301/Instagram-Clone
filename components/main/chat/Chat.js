...
import { getFirestore, collection, query, limit, doc, where, addDoc, getDocs, getDoc, FieldValue, orderBy,onSnapshot,updateDoc } from 'firebase/firestore';
...
function Chat(props) {
    ...
    const onSend =async () => {
        ...
        props.sendNotification(user.notificationToken, "New Message", textToSend, { type: "chat", user: auth.currentUser.uid })
    }
        ...
        <View style={[container.container, container.alignItemsCenter, utils.backgroundWhite]}>
            <Provider>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={[utils.padding10, container.container, item.creator == auth.currentUser.uid ? container.chatRight : container.chatLeft]}>
