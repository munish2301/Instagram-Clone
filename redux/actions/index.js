...
import { getFirestore, collection, query, limit, doc, where, getDocs, getDoc, deleteDoc, orderBy,onSnapshot,updateDoc } from 'firebase/firestore';
...
export function fetchUserChats() {
    return ((dispatch) => {
        const q=query(collection(db,'chats'),where("users", "array-contains", auth.currentUser.uid),orderBy("lastMessageTimestamp", "desc"));
...
export function queryUsersByUsername(username) {
    return ((dispatch, getState) => {
        return new Promise(
            ...
            const q=query(usersCollectionRef,where('username', '>=', username),limit(10));
            try{
                const snapshot= await getDocs(q);
